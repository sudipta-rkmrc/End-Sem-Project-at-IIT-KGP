...
# Instead of using create_model, import our custom Hiera builder.
#from my_models.sifar_hiera import Hiera
import my_models.sifar_hiera as sifar_hiera
...
def main(args):
    ...
    # Build the Hiera model with super image creation.
    #model = Hiera(num_classes=args.num_classes, duration=args.duration, 
     #                      super_img_rows=args.super_img_rows)
    
    #model = Hiera(num_classes=args.num_classes)
    model = sifar_hiera.hiera_base_224(num_classes=args.num_classes, pretrained = True, strict = False, duration = args.duration)
    model.to(device)
    print(model)
    #import ipdb;
