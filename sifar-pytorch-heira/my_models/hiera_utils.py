...
def interpolate_pos_embed(state_dict, new_size):
    pos_embed = state_dict["model_state"]["pos_embed"]  
    
    # Get the original and new grid sizes
    num_patches_old = int(pos_embed.shape[1] ** 0.5)  
    num_patches_new = 168 #int(new_size / model.patch_embed.patch_size)  
    ...
    if num_patches_old != num_patches_new:
        ...
        state_dict["model_state"]["pos_embed"] = torch.nn.Parameter(pos_embed)
    ...
    def inner(model_func: Callable) -> Callable:
        def model_def(pretrained: bool = False, checkpoint: str = default, strict: bool = True, **kwdargs) -> nn.Module:
            if pretrained:
                ...
                if "head.projection.weight" in state_dict["model_state"]:
                    ...
                    # If the user specified a different number of classes, remove the projection weights or else we'll error out
                        ...
                        # Resize the weight and bias to match the new number of classes
                        # Initialize new weights randomly (or any other method you prefer, e.g., from a normal distribution)
                        #new_weight = torch.nn.init.xavier_uniform_(torch.empty(kwdargs["num_classes"], state_dict["model_state"]["head.projection.weight"].shape[1]))
                        #new_bias = torch.zeros(kwdargs["num_classes"])

                        # Assign the new weights and biases to the state_dict
                       # state_dict["model_state"]["head.projection.weight"] = new_weight
                        #state_dict["model_state"]["head.projection.bias"] = new_bias
                        #print("Checkpoint", checkpoints)
                        #exit(0)
           ...
           # print(state_dict['model_state'].keys())
            print("Before = ",(state_dict["model_state"]["pos_embed"]).shape)
           

            state_dict = interpolate_pos_embed(state_dict,672)
            
            print("After = ",(state_dict["model_state"]["pos_embed"]).shape)
            ...
            if pretrained:
                ...
                msg =  model.load_state_dict(state_dict["model_state"], strict=strict)
                print(msg)
               # exit(0)
